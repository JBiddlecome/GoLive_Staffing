...
class ProfitPayload(BaseModel):
    ...
    client_toggles: Optional[dict[str, bool]] = None
    position_overrides: Optional[dict[str, dict[str, float]]] = None
    wc_fee_overrides: Optional[dict[str, float]] = None
    fee_toggles: Optional[dict[str, bool]] = None
...
async def get_profit_data(payload: ProfitPayload):
    ...
    try:
        ...
        sql = text(
            ...
            SELECT
                ...
                p.description AS shift_position_name,
                t.client_worked,
                t.employee_worked,
    ...
    # Extract raw collections for the UI before overrides
    ui_clients = sorted(df["client_name"].dropna().unique().tolist())
    
    ui_positions = []
    if "shift_position_name" in df.columns:
        pos_group = df.groupby("shift_position_name").agg({
            "pay_rate": "mean",
            "bill_rate": "mean"
        }).reset_index()
        for _, r in pos_group.iterrows():
            if pd.notna(r["shift_position_name"]) and str(r["shift_position_name"]).strip():
                pos_name = str(r["shift_position_name"])
                pr = float(r["pay_rate"])
                br = float(r["bill_rate"])
                
                # Apply overrides to the UI display data
                if payload.position_overrides and pos_name in payload.position_overrides:
                    override = payload.position_overrides[pos_name]
                    if "pay_rate" in override:
                        pr = float(override["pay_rate"])
                    if "markup_percentage" in override:
                        br = pr * (1 + (float(override["markup_percentage"]) / 100.0))
                
                markup = ((br - pr) / pr * 100) if pr > 0 else 0
                ui_positions.append({
                    "name": pos_name,
                    "pay_rate": round(pr, 2),
                    "bill_rate": round(br, 2),
                    "markup_percentage": round(markup, 2)
                })
        ui_positions.sort(key=lambda x: x["name"])
        
    ui_wc_fees = []
    if "wc_id" in df.columns and "wc_name" in df.columns and "wc_rate" in df.columns:
        wc_group = df.groupby(["wc_id", "wc_name"]).agg({"wc_rate": "first"}).reset_index()
        for _, r in wc_group.iterrows():
            if pd.notna(r["wc_id"]) and str(r["wc_id"]).strip():
                ui_wc_fees.append({
                    "wc_id": str(r["wc_id"]),
                    "wc_name": str(r["wc_name"]),
                    "wc_rate": round(float(r["wc_rate"]) * 100, 4) # convert to %
                })
        ui_wc_fees.sort(key=lambda x: x["wc_name"])
    ...
    def process_row(row):
        ...
        if payload.client_toggles and client_name in payload.client_toggles:
            if not payload.client_toggles[client_name]:
                return None  # Skip this row entirely
                
        bill_rate = float(row["bill_rate"])
        pay_rate = float(row["pay_rate"])
        
        position_name = str(row.get("shift_position_name", ""))
        if payload.position_overrides and position_name in payload.position_overrides:
            override = payload.position_overrides[position_name]
            if "pay_rate" in override:
                pay_rate = float(override["pay_rate"])
            if "markup_percentage" in override:
                bill_rate = pay_rate * (1 + (float(override["markup_percentage"]) / 100.0))
        ...
        if payload.wc_fee_overrides and wc_id in payload.wc_fee_overrides:
            wc_rate = float(payload.wc_fee_overrides[wc_id]) / 100.0

        msp_fee = total_bill * msp_rate
        wc_fee = total_pay * wc_rate
        
        if payload.fee_toggles:
            if not payload.fee_toggles.get("msp", True): msp_fee = 0.0
            if not payload.fee_toggles.get("wc", True): wc_fee = 0.0
        ...
        if payload.fee_toggles and not payload.fee_toggles.get("cc", True):
            cc_fee = 0.0
        ...
        if payload.fee_toggles and not payload.fee_toggles.get("bt", True):
            bt_fee = 0.0
        ...
        if payload.fee_toggles and not payload.fee_toggles.get("comm", True):
            commissions = 0.0
    ...
    if payload.fee_toggles and not payload.fee_toggles.get("tax", True):
        payroll_tax = 0.0
...
# ── FORECASTING PROFILES SYSTEM (JSON PERSISTENCE) ──────────────────

FORECASTING_PROFILES_PATH = _resolve_data_dir() / "forecasting_profiles.json"

def _load_profiles() -> dict:
    if FORECASTING_PROFILES_PATH.exists():
        try:
            with open(FORECASTING_PROFILES_PATH, "r") as f:
                return json.load(f)
        except (json.JSONDecodeError, OSError):
            pass
    return {}
...
def _save_profiles(profiles: dict) -> None:
    FORECASTING_PROFILES_PATH.parent.mkdir(parents=True, exist_ok=True)
    tmp = FORECASTING_PROFILES_PATH.with_suffix(".tmp")
    with open(tmp, "w") as f:
        json.dump(profiles, f, indent=2)
    tmp.replace(FORECASTING_PROFILES_PATH)

@router.get("/api/profiles")
async def get_profiles():
    profiles = _load_profiles()
    # Return just the keys (names) and metadata if we had any, for the dropdown
    return JSONResponse([{"name": k} for k in profiles.keys()])
...
async def get_profile(profile_name: str):
    ...
    return JSONResponse(profiles[profile_name])
...
async def save_profile(payload: SaveProfilePayload):
    if not payload.name or not payload.name.strip():
        raise HTTPException(status_code=400, detail="Profile name cannot be empty")
        
    profiles = _load_profiles()
    profiles[payload.name.strip()] = {
        "client_toggles": payload.client_toggles or {},
        "position_overrides": payload.position_overrides or {},
        "wc_fee_overrides": payload.wc_fee_overrides or {},
        "fee_toggles": payload.fee_toggles or {}
    }
    _save_profiles(profiles)
    return JSONResponse({"status": "ok", "name": payload.name.strip()})
...
async def export_to_excel(payload: ExportPayload):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        # Sheet 1: Client Breakdown
        if payload.client_breakdown:
            df_cb = pd.DataFrame(payload.client_breakdown)
            ...
            df_cb.rename(columns={
                ...
                "profit": "Gross Profit",
                "has_bt_fee": "Has BT Fee"
            }, inplace=True)
            df_cb.to_excel(writer, sheet_name="Client Breakdown", index=False)
        else:
            pd.DataFrame([{"Message": "No data"}]).to_excel(writer, sheet_name="Client Breakdown", index=False)
            
        # Sheet 2: Position Overrides
        if payload.ui_positions:
            df_po = pd.DataFrame(payload.ui_positions)
            ...
            df_po.to_excel(writer, sheet_name="Position Overrides", index=False)
            
        # Sheet 3: WC Fees Override
        if payload.ui_wc_fees:
            df_wc = pd.DataFrame(payload.ui_wc_fees)
            df_wc.rename(columns={
                "wc_id": "WC Code",
                "wc_name": "Description",
                "wc_rate": "WC Fee %"
            }, inplace=True)
            df_wc.to_excel(writer, sheet_name="WC Fees Override", index=False)
            
        # Sheet 4: Client Toggles
        if payload.client_toggles is not None:
            toggles_list = [{"Client Name": k, "Included": ("Yes" if v else "No")} for k, v in payload.client_toggles.items()]
            df_ct = pd.DataFrame(toggles_list)
            if df_ct.empty:
                pd.DataFrame([{"Message": "No clients"}]).to_excel(writer, sheet_name="Client Toggles", index=False)
            else:
                df_ct.to_excel(writer, sheet_name="Client Toggles", index=False)
        else:
            pd.DataFrame([{"Message": "No toggles provided"}]).to_excel(writer, sheet_name="Client Toggles", index=False)
            
    output.seek(0)
    
    headers = {
        'Content-Disposition': 'attachment; filename="Forecasting_Report.xlsx"'
    }
    return Response(

...
def main():
    ...
    try:
        ...
        with engine.connect() as conn:
            df = pd.read_sql("SELECT id, reason FROM status_reason WHERE type='cancel_reason'", conn)
            print("--- CANCEL REASONS ---")
            print(df.to_string())

            df2 = pd.read_sql("SHOW COLUMNS FROM publish_employee", conn)
            print("--- PUBLISH EMPLOYEE COLUMNS ---")
            print(df2.to_string())

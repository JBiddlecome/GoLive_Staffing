...
def run_test_simulation(start_date, end_date, reduction_amount, markup_percent, msp_filter):
    ...
    with engine.begin() as connection:
        ...
        try:
            ...
            asp_rules = [dict(r) for r in connection.execute(asp_sql).mappings().fetchall()]
        except Exception:
            asp_rules = []
    ...
    print("Baseline:")
    print(f"  Total Bill:  ${b_bill:,.2f}")
    print(f"  Gross Pay:   ${b_pay:,.2f}")
    print(f"  MSP Fee:     ${b_msp:,.2f} ({(b_msp/b_bill*100) if b_bill>0 else 0:.2f}%)")
    print(f"  Net Profit:  ${b_profit:,.2f}")
    
    print("Simulated:")
    print(f"  Total Bill:  ${s_bill:,.2f}")
    print(f"  Gross Pay:   ${s_pay:,.2f}")
    print(f"  MSP Fee:     ${s_msp:,.2f} ({(s_msp/s_bill*100) if s_bill>0 else 0:.2f}%)")
    print(f"  Net Profit:  ${s_profit:,.2f}")
    print(f"  Profit Gain: ${s_profit-b_profit:+,.2f} ({((s_profit/b_profit)-1)*100:+.2f}%)")

if __name__ == "__main__":
    # Test 1: Compass MSP, $2 reduction, 77% markup (default)
    run_test_simulation("2026-04-25", "2026-05-24", 2.0, 77.0, "2")
    
    # Test 2: No MSP (Direct Clients Only), $1.50 reduction, 80% markup
    run_test_simulation("2026-04-25", "2026-05-24", 1.5, 80.0, "none")

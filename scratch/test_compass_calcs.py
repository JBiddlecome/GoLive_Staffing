...
def test_calcs():
    ...
    def process_row_sim(row, simulate=False):
        ...
        if simulate:
            # Pay rates reduced by $2, no less than min_wage. No lock columns in DB, so lock = False
    ...
    print(f"MSP Fee:    ${base_msp:,.2f}")
    print(f"WC Fee:     ${base_wc:,.2f}")
    print(f"CC Fee:     ${base_cc:,.2f}")
    print(f"BT Fee:     ${base_bt:,.2f}")
    print(f"Comm:       ${base_comm:,.2f}")
    print(f"Tax (10%):  ${base_tax:,.2f}")
    print(f"Profit:     ${base_profit:,.2f}")

    print("\n--- SIMULATED CALCULATIONS (-$2 Pay, 1.77x Bill) ---")
    ...
    print(f"Comm:       ${sim_comm:,.2f}")
    print(f"Tax (10%):  ${sim_tax:,.2f}")
